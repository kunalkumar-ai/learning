...
def process_data(file_name):
    ...
    words = re.findall(r'\w+', content_lowercase)
    ### END CODE HERE ###
...
def switch_letter(word, verbose=False):
    ...
    Input:
        word: input string
    Output:
        switch_l: a list of all possible strings with one adjacent character switched
    ''' 
    
    switch_l = []  # List to store results
    split_l = []   # List to store splits
    ...
    # Perform switching on valid splits
    for L, R in split_l:
        if len(R) > 1:  # Ensure there are at least two characters in R to switch
            switched = L + R[1] + R[0] + R[2:]  # Swap R[0] and R[1], append the rest of R
...
def replace_letter(word, verbose=False):
    ...
    replace_l = []  # List to store results
    split_l = []    # List to store splits
    ...
    for L, R in split_l:
        if R:  # Ensure R is not empty
            for char in letters:  # Replace with every letter in the alphabet
                if char != R[0]:  # Avoid replacing with the same letter
                    replace_l.append(L + char + R[1:])
    ### END CODE HERE ###
...
def insert_letter(word, verbose=False):
    ...
    for i in range(len(word) + 1):
        split_l.append((word[:i], word[i:]))  
    for L, R in split_l:
        for char in letters:  # Insert each letter of the alphabet
            insert_l.append(L + char + R)
    ### END CODE HERE ###
...
def edit_one_letter(word, allow_switches=True):
    ...
    edit_one_set = set()  # To store all unique edits
    
    ### START CODE HERE ###
    # Generate all possible edits
    deletes = delete_letter(word)  # Delete one letter
    inserts = insert_letter(word)  # Insert one letter
    replaces = replace_letter(word)  # Replace one letter
    
    if allow_switches:  # Include switches if allowed
        switches = switch_letter(word)
    else:
        switches = set()
    
    # Combine all edits into a single set
    edit_one_set = set(deletes) | set(inserts) | set(replaces) | set(switches)
    ### END CODE HERE ###
...
def edit_two_letters(word, allow_switches=True):
    ...
    Input:
        ...
        allow_switches: a boolean flag to allow adjacent letter swaps
    Output:
        edit_two_set: a set of words with two possible edits.
    """
    
    edit_two_set = set()
    
    ### START CODE HERE ###
    # Generate first edits
    one_edits = edit_one_letter(word, allow_switches=allow_switches)
    
    # Generate second edits
    for edit in one_edits:
        two_edits = edit_one_letter(edit, allow_switches=allow_switches)
        edit_two_set.update(two_edits)  # Add to the final set
    ### END CODE HERE ###
    
    # return the set of all two edits
    return edit_two_set
...
def get_corrections(word, probs, vocab, n=2, verbose=False):
    ...
    suggestions = (one_edits & vocab) or (two_edits & vocab) or {word}
    
    # Step 2: Create a dictionary of suggestions with their probabilities
    best_words = {w: probs.get(w, 0) for w in suggestions}
    
    # Step 3: Get the n most probable suggestions
    from collections import Counter
...
def min_edit_distance(source, target, ins_cost=1, del_cost=1, rep_cost=2):
    ...
    ### START CODE HERE ###
    # Fill in column 0 (deleting all characters from the source)
    for row in range(1, m+1):
        D[row, 0] = row * del_cost
    
    # Fill in row 0 (inserting all characters into the source)
    for col in range(1, n+1):
        D[0, col] = col * ins_cost
    
    # Loop through the matrix to compute costs
    for row in range(1, m+1):
        for col in range(1, n+1):
            # Intialize r_cost to the 'replace' cost
            r_cost = rep_cost
            
            # Check if the current characters in source and target are the same
            if source[row-1] == target[col-1]:
                r_cost = 0  # No cost if characters match
            
            # Compute the minimum cost from the three operations
            D[row, col] = min(
    ...
    # Minimum edit distance is the value in the bottom-right corner of the matrix
    med = D[m, n]
    ### END CODE HERE ###
